...
import { CommentOutlined, ProductOutlined,FileProtectOutlined,SafetyCertificateOutlined,FileExclamationOutlined,CreditCardOutlined,PlusCircleOutlined } from '@ant-design/icons';
...
const Home = () =>{
        ...
        try {
            ...
            axios.get('http://localhost:8000/ticket/').then((res)=>{
              ...
              res.data.map(async(r:any)=>{
                ...
                x.answer_status = ticketAnswers.find((t:any)=>t.id===r.ticket) as boolean
                d.push(x)
              })
              setData(d)
              setTickets(res.data)
    ...
    const columns: TableColumnsType<DataType> = [
      ...
      render: (_, { tags }) => (
       ...
       render: (_,answer_status) => (
        answer_status?<Badge status="success" text="Answerd" />
        :<Badge status="default" text="Unanswerd" />
      ),
       },
